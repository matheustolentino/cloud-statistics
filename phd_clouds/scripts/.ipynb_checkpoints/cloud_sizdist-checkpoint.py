...
for i in range(50):
    ...
    if dist.sum() > 10:
        ...
        if ind_maxs:
            ...
            n_montains = len(ind_maxs)

            if ind_global_max > ind_maxs[0]:
                # dist = moving_average(dist, n_montains-1)
                # diameter = moving_average(diameter, n_montains-1)
                ind_to_exclude = np.where(ind_maxs < ind_global_max)[0][-1]
                dist = dist[ind_to_exclude+1:]
                diameter = diameter[ind_to_exclude+1:]
                # dist = savgol_filter(dist, 9, 3)
                n_montains = len(find_mountains(dist, 0.001))
        # if n_montains > 1:
        ...
        # plt.show()
        #**************************************************************************************************
        f = interpolate.interp1d(diameter, dist, kind='cubic')
        ...
        # if len(valleys) > 1 and y2_test.sum() > 10:
        #     dist_new = savgol_filter(dist_new, 601, 3)
        #     # dist_new = moving_average(dist_new, 201)
        #     # diameter_new = moving_average(diameter_new, 201)

        if n_montains == 1 or ind_sep_x2 != 0:
            valleys  = find_valleys(np.diff(y2_test[:ind_sep_x2]))
        
        # if len(valleys) > 1:
        #     # dist_new = savgol_filter(dist_new, 101, 3)
        #     dist_new = moving_average(dist_new, 201)
        #     diameter_new = moving_average(diameter_new, 201)
        ...
        f0       = axs[0].plot(standard_diameter, data[i, 8:]/integrate.simps(data[i, 8:],standard_diameter ) , 'ob')
        ...
        if num_x1_original > 3 and data[i, 8:].sum() > 10:
            if y1_test.sum() > 10 and len(valleys) == 0 and n_montains == 1: # If using interpolated distribution
                ...
                axs[0].set_title("sum y2 original = %d, n_montain = %.3f, n2/n0 = %.3f"%(y2_test.sum(), n_montains, n2/n0))
