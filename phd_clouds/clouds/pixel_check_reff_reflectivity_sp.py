...
# changes to inplement:
# 1. for mixed and liquid clouds, use this code to identify doppler spectra for liquid and ice clouds
# 2. analisas skweness, kurtosis, spectrasl width and phase difference(Lv1 variabkle)
# 3. use this code to identify the number of modes in the doppler spectra


script_dir = '/home/matheustolen/Documentos/matheus_doctorado/phd-clouds/phd_clouds/clouds'
os.chdir(script_dir)
PATH_FIG          = '../figures/'
...
date = datetime.datetime.strptime('2023-11-28', '%Y-%m-%d') # el codigo de cris funciona para chirps en esta fecha
...
mask_ice_droplets = ds_classification['target_classification'] == 5
time_mask         = np.max(mask_ice_droplets, axis=1)
time_ice_droplets = ds_classification.time[time_mask]
...
sdata   = ds_classification['target_classification'].sel(time=slice(time_ice_droplets[0], time_ice_droplets[-1]))
sreff   = ds_diff_der.sel(time=slice(time_ice_droplets[0], time_ice_droplets[-1]))
...
pc = ax1.pcolormesh(sreff.time, sreff.height, sreff.T, shading='auto', vmin=0, vmax=sreff.max(), cmap='jet')
ax1.set_title(sreff.attrs['long_name'])
...
pc2 = ax2.pcolormesh(sdata.time, sdata.height, sdata.T,
...
ranges_to_study = np.arange(1000, 7000, 60)
#-------------------------------------------------------------------------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------------------------------------------------------------------------
time_guess_chriss = np.datetime64('2023-11-28T19:40:15.000915527') ##CHANGE THIS
...
# add_all_products_from_LV1(gpy_radarnc.data, gpy_radarnc.band) # change for LV1 data


# change gpy_radarnc.data by test:
# gpy_radarnc._data = gpy_radarnc.data.resample(time='30S').mean()
# ranges_to_study = np.arange(100, 10000, 100)
if len(ranges_to_study) > 1:
    fig, filepath =  gpy_radarnc.plot_spectra_by_range(target_time=time_to_study, 
                                                   ...
                                                   **{"savefig": False, "velocity_limits": (-1, .25)}
                                                   )
# else:
