...
def plot_microphysics_evolution2(sliced_microphys, 
    ...
    for season in list_seasons:
        mean_profile = mean_by_season.sel(season=season)
        ax2.plot(mean_profile.where(mean_profile < 300, drop=True), mean_profile.height / 1e3, label=f"{season}")
        ax2b.plot(mean_profile.where(mean_profile > 1000, drop=True), mean_profile.height / 1e3, label=f"{season}")
    
    ax2.set_xlabel(f"{sliced_microphys.attrs['long_name']} ({sliced_microphys.attrs['units']})")
    ax2.set_ylabel("Height (km)")
    ax2.set_xlim(0, 300)
    ax2b.set_xlim(1000, max_value + .1)
...
def read_microphysics(filepath_microphys, get_var, endswith):
    # Get the list of netCDF files in the specified directory
...
for cloud in clouds:
    ...
    for i, variable in enumerate(variables):
        ...
        fig_axes = get_fig_axes(variable)
        if chunked_integrated is not None:
            ...
            plot_microphysics_evolution2(sliced_microphys, 
                                                           ...
                                                           var_short_name=variable,
                                                           axes=fig_axes)
    fig.savefig(f"{PATH_FIG}monthly_evolution_for_{cloud}_clouds.png", dpi=400, bbox_inches='tight')
    plt.show()